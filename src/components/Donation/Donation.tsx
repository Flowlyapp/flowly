...
export default function BuyMeABeer() {
  ...
  const handleDonateClick = async () => {
    if (selectedOption === 'TON') {
      if (wallet) {
        ...
        const bloc = await tonConnectUi.sendTransaction(transferPayload)
  ...
  return (
    <div style={{ paddingBottom: '24px' }}>
      <Input header="Your name" value={name} onChange={handleNameChange} />
      ...
      <Textarea header="Message" placeholder="Text max 240 characters" value={message} onChange={handleMessageChange} />
